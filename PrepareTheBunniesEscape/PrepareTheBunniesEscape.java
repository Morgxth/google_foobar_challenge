...
public class PrepareTheBunniesEscape {
    ...
    public static int solution(int[][] map) {
        ...
        q.add(new node(0,0));
        cost[0][0]=1;
        sizeCheck(map, n, m, dx, dy, cost, (Queue<node>) q);

        q.add(new node(n-1,m-1));
        cost2[n-1][m-1]=1;
        sizeCheck(map, n, m, dx, dy, cost2, (Queue<node>) q);
        int ans=10000;
        for(i=0;i<n;i++) {
            for(j=0;j<m;j++) {
                if(map[i][j]==1) {
    ...
    private static void sizeCheck(int[][] map, int n, int m, int[] dx, int[] dy, int[][] cost2, Queue<node> q) {
        int i;
        while(q.size()>0) {
            new node();
            node nn;
